...
// ==========================================//  ✅ ПОКАЗ СТРАНИЦ (ОБНОВЛЕНО: ФОТО + ЛОГО)
// ==========================================
function showPage(pageId) {
  ...
  if (pageId === 'home') {
    ...
    if (data && targetPage) {
      ...
      // ✅ ФОТО ДЛЯ СТРАНИЦЫ "ОБО МНЕ" (С ФОЛБЭКОМ НА ЛОГО)
      if (pageId === 'about') {
        let imageSrc = config.branding?.agentPhoto;
        // Если нет фото агента, берем логотип
...
function openDirectChat() {  const username = currentAgentData.telegramUsername || '';
  if (username) {
...
function renderFilters() {
  ...
  document.querySelectorAll('.price-btn').forEach(btn => {    btn.addEventListener('click', function() {
...
function initMap() {
  ...
  filterListings();  setTimeout(() => map.invalidateSize(), 150);
}
...
function openDetails(id) {
  ...
  document.getElementById('modalMeta').innerHTML = `
    ...
    <div class="meta-row"><span>🔨 Отделка: ${escapeHtml(item.finishing) || ''}</span></div>    <div class="meta-row"><span>📅 Срок сдачи: ${escapeHtml(item.completion_soonest) || ''}${item.completion_soonest && item.completion_all ? ' - ' : ''}${escapeHtml(item.completion_all) || ''}</span></div>`;
...
function initTelegramMask() {
  ...
  input.addEventListener('input', function(e) {
    ...
    if (val.length > 32) val = val.slice(0, 32);    e.target.value = val;
  });
}
