// ==========================================
// ГЛОБАЛЬНЫЕ ПЕРЕМЕННЫЕ И НАСТРОЙКИ YANDEX
// ==========================================
let config = {};
let listings = [];
...
// Функция конвертации ссылки GitHub -> Yandex
...
function showPage(pageId) {
    ...
    if (pageId === 'home') {
        ...
        const data = pagesData[pageId];        const targetPage = document.getElementById(`page-${pageId}`);
        if (data && targetPage) {
...
function renderContactsPage() {
    ...
    if (agentPhoto && agentPhoto.trim() && agentPhoto !== 'logo.png') {
        ...
        img.src = yandexPhoto;
        img.alt = data.name || 'Агент';        img.onerror = function() { this.onerror = null; this.src = agentPhoto; avatarEl.innerHTML = '👤'; };
...
function toggleFilters() {
    ...
    const btn = document.querySelector('.filters-toggle-btn');    block.classList.toggle('hidden');
    ...
    Promise.all([
        ...
        fetch(`${config.supabase.url}/functions/v1/notify-lead`, {
            ...
            headers: {
                ...
                'Authorization': `Bearer ${config.supabase.anonKey}`,                'apikey': config.supabase.anonKey
            },
