...
async function loadData() {
  try {
    ...
    document.getElementById('listings').innerHTML =
      '<div class="loader">Ошибка загрузки. Обновите страницу.</div>';
  }
}

function parseCSV(csv) {  const rows = [];
...
function renderListings(items) {
  ...
  container.innerHTML = items.map(item => `
    <article class="card" onclick="openModal('${item.id}')">
      <img src="${item.image_main}" class="card-img" loading="lazy">
      <div class="card-body">
        ...
        <div class="card-price">${formatPriceMillions(item.price_from)}</div>
        <div class="card-meta">
...
function openModal(id) {
  ...
  document.getElementById('modalPrice').textContent = formatPriceFull(item.price_from);
  ...
  contactBtn.href = APP_CONFIG.brand.contactLink;
  contactBtn.target = '_blank';
  contactBtn.onclick = null; // Сброс старых обработчиков
...
// Форматирование для карточки (14.6 млн ₽)
function formatPriceMillions(num) {
  if (!num) return '0 ₽';
  const millions = num / 1000000;
  return `${millions.toFixed(1)} млн ₽`;
}

// Полное форматирование для модалки (14 600 000 ₽)
