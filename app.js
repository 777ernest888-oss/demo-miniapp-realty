...
// ==========================================
//  YANDEX STORAGE - КОНВЕРТАЦИЯ ССЫЛОК
// ==========================================
const YANDEX_STORAGE_BASE = 'https://storage.yandexcloud.net/property-images/';
const GITHUB_RAW_BASE = 'https://raw.githubusercontent.com/777ernest888-oss/demo-miniapp-realty/main/property-images/';
...
function getImageUrl(sourceUrl) {
    ...
    if (sourceUrl.includes('raw.githubusercontent.com')) {
        return sourceUrl.replace(GITHUB_RAW_BASE, YANDEX_STORAGE_BASE);
    }
...
// ✅ ЗАГРУЗКА ИЗ SUPABASE (единственный источник для объектов)async function loadPropertiesFromSupabase() {
...
function hideBack() {  const btn = document.getElementById('headerBackBtn');
...
targetPage.querySelector('.page-content').innerHTML = data.content;
  if (pageId === 'about') {
    let imageSrc = config.branding?.agentPhoto;
    if (!imageSrc && config.branding?.logo && config.branding.logo !== 'logo.png') {
      imageSrc = config.branding.logo;
    }
    if (imageSrc) {
      const contentDiv = targetPage.querySelector('.page-content');      const img = document.createElement('img');
      const yandexSrc = getImageUrl(imageSrc);
      img.src = yandexSrc;
      img.className = 'about-agent-photo';
      img.alt = 'Фото';
      img.onerror = function() { this.onerror = null; this.src = imageSrc; };
...
avatarEl.innerHTML = '';
const agentPhoto = config.branding?.agentPhoto;
if (agentPhoto && agentPhoto.trim() && agentPhoto !== 'logo.png') {
const img = document.createElement('img');
const yandexPhoto = getImageUrl(agentPhoto);
img.src = yandexPhoto;
img.alt = data.name || 'Агент';
img.onerror = function() { this.onerror = null; this.src = agentPhoto; avatarEl.innerHTML = '👤'; };
avatarEl.appendChild(img);
} else if (config.branding?.logo && config.branding.logo !== 'logo.png') {
const img = document.createElement('img');
const yandexLogo = getImageUrl(config.branding.logo);
img.src = yandexLogo;
img.alt = 'Логотип';
img.onerror = function() { this.onerror = null; this.src = config.branding.logo; };
avatarEl.appendChild(img);
} else {
avatarEl.innerHTML = '👤';
}const hasAgency = data.agencyName || data.agencyAddress;  document.getElementById('agencyBlock').style.display = hasAgency ? 'block' : 'none';
...
await loadAgentData();
await loadPagesData();
// ✅ Только Supabase — без фолбэка на Google Sheets
let propertiesData = await loadPropertiesFromSupabase();
if (!propertiesData || propertiesData.length === 0) {
  console.warn('⚠️ No properties in Supabase');
  propertiesData = [];
}
...
if (headerLogo && config.branding.logo) {
  ...
  headerLogo.onerror = function() { this.onerror = null; this.src = config.branding.logo; };
}
}
...
const itemRooms = String(item.rooms).split(',').map(r => r.trim());      if (!selectedRooms.some(r => itemRooms.includes(r))) return false;
}
return true;
});
...
if (!container) return;
container.innerHTML = '';if (listings.length === 0) {
...
const fallbackMain = item.image_main || '';

const card = document.createElement('div');
card.className = 'listing-card';
card.onclick = (e) => { if (!e.target.closest('.consult-btn-inline')) openDetails(item.id); };
card.innerHTML = `
   <img src="${escapeHtml(yandexMain)}" alt="${escapeHtml(item.name) || ''}" class="listing-image" onerror="this.onerror=null;this.src='${escapeHtml(fallbackMain)}';">
...
document.getElementById('modalMeta').innerHTML = `<div class="meta-row"><span>📍 ${escapeHtml(item.address) || ''}</span></div><div class="meta-row"><span>🚇 м. ${escapeHtml(item.metro) || ''}</span></div><div class="meta-row"><span>🏗 Класс: ${escapeHtml(item.class) || ''}</span></div><div class="meta-row"><span>🔨 Отделка: ${escapeHtml(item.finishing) || ''}</span></div><div class="meta-row"><span>📅 Срок сдачи: ${escapeHtml(item.completion_soonest) || ''}${item.completion_soonest && item.completion_all ? ' - ' : ''}${escapeHtml(item.completion_all) || ''}</span></div>`;
...
if (item.images_gallery) {allImages = allImages.concat(item.images_gallery.split(',').map(u => u.trim()).filter(u => u));
}
...
allImages.forEach((url, index) => {
  ...
  img.src = yandexUrl;
  img.onclick = () => window.open(yandexUrl, '_blank');
...
plansImages.forEach(url => {  const slide = document.createElement('div');
  slide.className = 'slide';
...
document.getElementById('consultModal').classList.add('hidden');  document.getElementById('consultForm').reset();
